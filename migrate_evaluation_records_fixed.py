...
def migrate_evaluation_records():
    ...
    with app.app_context():
        try:
            ...
            # 获取源表的字段名和类型
            source_columns = inspector.get_columns('evaluation_record')
            source_column_names = [col['name'] for col in source_columns]
            print(f"源表'evaluation_record'的字段: {source_column_names}")
            ...
            for record in source_data:
                ...
                for i, column in enumerate(source_columns):
                    ...
                    if col_name in target_columns and col_name != 'id':  # 跳过id字段
                        # 处理日期时间字段
                        if 'datetime' in col_type.lower() and value is not None:
                            try:
                                # 尝试将字符串转换为datetime对象
                                if isinstance(value, str):
                                    # 假设日期格式为 'YYYY-MM-DD HH:MM:SS.ffffff'
                                    value = datetime.strptime(value.split('.')[0], '%Y-%m-%d %H:%M:%S')
                                setattr(new_record, col_name, value)
                            except Exception as e:
                                print(f"转换日期时间字段 '{col_name}' 时出错: {str(e)}")
                                print(f"字段值: {value}")
                        else:
                            setattr(new_record, col_name, value)
